//! Block-paged KV-cache attention (a.k.a. PagedAttention).
//!
//! This is a device-agnostic, pure-`candle` reference implementation of the
//! attention mechanism popularised by vLLM, where the key/value cache is not a
//! single contiguous buffer per sequence but a pool of fixed-size *blocks*.
//! Each sequence keeps a *block table* mapping its logical block indices to
//! physical blocks in the shared pool, which makes it possible to:
//!
...
//!
//! Unlike the fused CUDA kernel used in production serving stacks, this
//! implementation is written entirely in terms of `candle` tensor operations
//! (`index_select` + dense attention), so it runs unchanged on CPU, CUDA and
//! Metal. It is meant as a correctness reference and a usable building block;
//! a fused kernel can be slotted in later behind the same API, exactly like
//! [`super::varlen`] mirrors the fused flash-attention path.
//!
//! Cache tensor layout (chosen for clarity):
//!
...
//!
//! See <https://github.com/huggingface/candle/issues/3649>.
...
/// KV cache. This matches the canonical PagedAttention *decode* signature.
///
/// **Shapes**
...
/// `h / (num_heads / num_kv_heads)` (the vLLM convention).
///
/// `alibi_slopes`, when provided, must have shape `[num_heads]` and adds the
/// standard ALiBi distance bias.
...
pub fn paged_attention(
    ...
    for seq in 0..num_seqs {
        ...
        let scale = Tensor::new(scale, device)?.to_dtype(scores.dtype())?;
        let mut scores = scores.broadcast_mul(&scale)?;
...
///
/// For a single (most recent) query token at position `ctx_len - 1`, the bias
/// applied to key position `j` is `-slope_h * (ctx_len - 1 - j)`.
fn alibi_bias(
    ctx_len: usize,
    num_heads: usize,
    alibi_slopes: &Tensor,
...
/// sequence is evicted, implementing the block-pooling memory strategy
/// requested for high-throughput serving. Allocation is O(1).
#[derive(Debug, Clone)]
